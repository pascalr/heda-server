...
import { getLocale, recipeOrTranslation } from "./lib"
...
export const Home = () => {
  ...
  const [translatedRecipes] = useState(gon.translated_recipes)
  const [recipes1,] = useState(gon.recipes1.map(r => recipeOrTranslation(r, translatedRecipes, locale)))
  const [recipes2,] = useState(gon.recipes2.map(r => recipeOrTranslation(r, translatedRecipes, locale)))
  const originalRecipe = useHcuState([gon.recipe], {tableName: 'recipes'})[0]

  const recipe = recipeOrTranslation(originalRecipe, translatedRecipes, locale)
